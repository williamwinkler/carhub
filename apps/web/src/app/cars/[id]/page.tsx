...
import { FaHeart, FaRegHeart, FaArrowLeft, FaUser, FaCalendar, FaPalette, FaTachometerAlt, FaDollarSign, FaCar, FaIndustry } from "react-icons/fa";
...
export default function CarDetailPage({ params }: CarDetailPageProps) {
  ...
  const carQuery = trpc.cars.getById.useQuery({ id: params.id });
  const toggleFavoriteMutation = trpc.cars.toggleFavorite.useMutation({
    onSuccess: () => {
      utils.cars.getById.invalidate({ id: params.id });
  ...
  const handleToggleFavorite = async () => {
    ...
    await toggleFavoriteMutation.mutateAsync({ id: params.id });
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-slate-800/50 backdrop-blur-sm border border-red-500/20 rounded-lg p-8 text-center">
            <h1 className="text-2xl font-bold text-red-400 mb-4">Car Not Found</h1>
            <p className="text-slate-400 mb-6">
              {carQuery.error?.message || "The car you're looking for doesn't exist or has been removed."}
            </p>
            <button
              onClick={handleBack}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-lg overflow-hidden">
          ...
          <div className="p-8 border-b border-slate-700/50">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-slate-400 text-lg">{car.year} • {car.color}</p>
                ...
                <div className="flex items-center gap-6 mt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-slate-300">
                      Manufacturer: <span className="text-blue-400 font-medium">{car.model?.manufacturer?.name}</span>
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <FaCar className="w-4 h-4 text-purple-400" />
                    <span className="text-sm text-slate-300">
                      Model: <span className="text-purple-400 font-medium">{car.model?.name}</span>
                    </span>
                  </div>
                </div>
              </div>
          ...
          {/* Details Grid */}
          <div className="p-8">
            <h2 className="text-2xl font-bold text-slate-200 mb-6">Vehicle Details</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-700/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-slate-200 text-lg font-semibold">{car.year}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-700/30 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-slate-200 text-lg font-semibold">{car.color}</p>
                </div>
              </div>
            ...
            {car.model?.manufacturer && (
              <div className="mt-8 p-6 bg-slate-700/20 rounded-lg">
                <h3 className="text-xl font-bold text-slate-200 mb-3">About {car.model.manufacturer.name}</h3>
                <p className="text-slate-400">
                  This vehicle is manufactured by {car.model.manufacturer.name},
                  known for their quality and reliability in the automotive industry.
                </p>
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-slate-200 font-medium">{car.model.manufacturer.name}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <FaCar className="w-4 h-4 text-purple-400" />
                    <div>
                      ...
                      <p className="text-slate-200 font-medium">{car.model.name}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-8 p-6 bg-gradient-to-r from-blue-600/10 to-purple-600/10 border border-blue-500/20 rounded-lg">
              <h3 className="text-xl font-bold text-slate-200 mb-3">Interested in this car?</h3>
              <p className="text-slate-400 mb-4">
                Contact the seller to learn more about this vehicle and arrange a viewing.
              </p>
